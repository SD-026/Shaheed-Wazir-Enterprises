...
const TransactionTrash = () => {
  ...
  useEffect(() => {
    const fetchDeletedTransactions = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get('http://localhost:5000/api/transactions/trash');
  ...
  const handleRestore = async (transactionId) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/transactions/restore/${transactionId}`);
  ...
  const handlePermanentDelete = async (transactionId) => {
      ...
      try {
        await axios.delete(`http://localhost:5000/api/transactions/deletepermanent/${transactionId}`);
